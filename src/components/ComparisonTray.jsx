...
const ComparisonTray = ({ selectedItems, onRemove, onCompare, onClear }) => {
    ...
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-10 duration-500">
            <div className="bg-slate-900 text-white rounded-3xl shadow-2xl shadow-blue-500/20 border border-slate-800 p-2 flex items-center gap-4 pl-6">
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-blue-600 rounded-xl">
                        <Columns size={20} className="text-white" />
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">비교하기</span>
                ...
                <div className="flex items-center gap-2 overflow-x-auto max-w-[300px] no-scrollbar py-1">
                    {selectedItems.map((item) => (
                        <div key={item.code} className="relative group shrink-0">
                            <div className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center p-1 border border-slate-700 hover:border-blue-500 transition-all overflow-hidden">
                                {item.imageUrl ? (
                                    <img src={item.imageUrl} alt={item.model} className="w-full h-full object-contain" />
                                ) : (
                                    <span className="text-[10px] font-bold text-slate-500">{item.brand[0]}</span>
                                )}
                            </div>
                            <button
                                onClick={() => onRemove(item.code)}
                                className="absolute -top-1 -right-1 bg-slate-700 hover:bg-red-500 text-white p-0.5 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all active:scale-95"
                            >
                                <X size={10} />
                            </button>
                        </div>
                    ...
                    {selectedItems.length < 4 && (
                        <div className="w-12 h-12 rounded-xl border-2 border-dashed border-slate-800 flex items-center justify-center text-slate-700 italic font-bold text-xs shrink-0">
                            +
                        </div>
                    )}
                </div>
